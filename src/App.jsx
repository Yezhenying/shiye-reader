...
import { createFullBackup, createLightBackup, inspectBackup, restoreBackup } from './backup.js';
...
function trashImpactDescription(impact) {
  const labels = { books: '书籍', files: '原文件', sections: '章节', progress: '进度', bookmarks: '书签', notes: '笔记', highlights: '划线', sessions: '阅读记录' };
  return Object.entries(labels).map(([key, label]) => impact[key] ? `${label} ${impact[key]}` : '').filter(Boolean).join('，') || '该项目';
}
...
function LibraryPage({ library, openImport, toast }) {
  ...
  const permanentlyRemove = async trash => { try { const impact = await library.inspectTrash(trash.id); if (!confirm(`将永久删除：${trashImpactDescription(impact)}。此操作不可恢复，是否继续？`)) return; const removed = await library.permanentlyDeleteTrash(trash.id); toast(`已永久删除 ${removed.total} 项数据`); } catch (error) { toast(`永久删除失败：${error.message}`); } };
...
function SettingsPage({ library, prefs, setPrefs, toast }) {
  const restoreRef=useRef(); const [busy,setBusy]=useState(false); const [restoreStrategy,setRestoreStrategy]=useState('REPLACE');
  ...
  const lightBackup=async()=>{setBusy(true);try{await createLightBackup();toast('轻量数据备份已生成；不含原文件与正文');}catch(e){toast(`轻量备份导出失败：${e.message}`)}finally{setBusy(false)}};
  const restore=async file=>{if(!file)return;setBusy(true);try{const preview=await inspectBackup(file);const counts=Object.entries(preview.manifest.stores).filter(([,count])=>count).map(([name,count])=>`${name} ${count}`).join('，');const modeMessage=preview.manifest.mode==='LIGHT'?'这是轻量备份，不含原文件、封面与正文；恢复后需重新导入原文件。':'这是完整可阅读备份。';if(!confirm(`${modeMessage}\n将恢复快照（${counts||'空快照'}）并原子替换当前本地数据。建议先导出当前快照。恢复期间其他拾页标签页将暂停写入。是否继续？`))return;await library.runWithWriteBarrier(()=>restoreBackup(file,{strategy:restoreStrategy}));toast('快照已恢复，其他标签页已刷新');}catch(e){toast(`恢复失败：${e.message}。现有数据库未被修改。`)}finally{setBusy(false)}};
  ...
  return <section className="module-page"><div className="module-heading"><span className="eyebrow">所有控件即时生效</span><h1>设置</h1><p>阅读外观、存储与备份。</p></div><div className="settings-card card"><div><strong>界面主题</strong><span>跟随你的阅读环境</span></div><div className="theme-segment" role="group" aria-label="界面主题">{[['light',Sun,'日间'],['dark',Moon,'夜间'],['sepia',BookOpen,'暖色']].map(([v,I,l])=><button aria-pressed={prefs.theme===v} className={prefs.theme===v?'active':''} key={v} onClick={()=>update({theme:v})}><I size={14}/>{l}</button>)}</div><div><strong>正文字号</strong><span>{prefs.fontSize}px</span></div><input aria-label="正文字号" type="range" min="14" max="26" value={prefs.fontSize} onChange={e=>update({fontSize:+e.target.value})}/><div><strong>行距</strong><span>{prefs.lineHeight}</span></div><input aria-label="正文行距" type="range" min="1.5" max="2.6" step="0.1" value={prefs.lineHeight} onChange={e=>update({lineHeight:+e.target.value})}/><div><strong>正文宽度</strong><span>{prefs.width}px</span></div><input aria-label="正文宽度" type="range" min="560" max="920" step="20" value={prefs.width} onChange={e=>update({width:+e.target.value})}/><div><strong>本地存储</strong><span>{used} · {library.storage.persisted?'已获持久化':'未获持久化'}</span></div><button onClick={async()=>{const ok=await requestPersistentStorage();await library.reload();toast(ok?'已获得持久存储':'浏览器未授予持久存储，请定期备份');}}>请求持久存储</button><div><strong>备份 schema v2</strong><span>完整备份包含草稿、封面与原文件；轻量备份仅保留书目、进度和笔记，恢复后需重新导入原文件。</span></div><label className="restore-strategy">恢复冲突处理<select aria-label="恢复冲突处理" value={restoreStrategy} onChange={event=>setRestoreStrategy(event.target.value)} disabled={busy}><option value="REPLACE">整体替换当前数据</option><option value="SKIP">跳过同 ID 冲突记录</option><option value="COPY">保留冲突记录为副本</option><option value="LATEST">按更新时间保留较新记录</option></select></label><div className="dialog-buttons"><button disabled={busy} onClick={backup}>导出完整备份</button><button disabled={busy} onClick={lightBackup}>导出轻量备份</button><button disabled={busy} onClick={()=>restoreRef.current?.click()}>恢复备份</button><input aria-label="选择备份文件" hidden ref={restoreRef} type="file" accept=".zip" onChange={e=>{const file=e.target.files?.[0];restore(file);e.target.value='';}}/></div></div></section>;
}
...
function ReaderContent({ book, library, prefs, toast, openNote, sections, file }) {
  ...
  if(book.sourceMissing||(!sections.length&&book.capability==='FILE_ONLY'))return <div className="reader-shell"><header><a href="#/library"><ArrowLeft/>返回书架</a><strong>{book.title}</strong><span/></header><div className="reader-unavailable"><FileText/><h1>{book.sourceMissing?'轻量备份未包含原文件':'此格式仅保存原文件'}</h1><p>{book.sourceMissing?'请重新导入此书的原文件后再阅读。':'MOBI/AZW3 或固定版式 EPUB 暂不支持正文阅读，请转换为流式 EPUB、PDF、TXT 或 Markdown。'}</p></div></div>;
